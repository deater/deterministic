...
int main(int argc, char **argv) {
   ...
   printf("(* \t%s %s *)\n",bench_name,input);   
	 ...
	 if (our_stats[benchmark].valid[i][j]) {
	    printf("%lld ",our_stats[benchmark].instrs[i][j]);
	 }
      }
      printf("mean=%lld ",our_stats[benchmark].mean[i]);
      printf("std=%lf *)\n",stdev);  

   printf("(* bench_mean=%lld dev=%lf *)\n",our_stats[benchmark].bench_mean,
	  stdev);
#endif
...
#define LEGEND_YSCALE 1.0
       ...
       /* y-axis */
    printf("\n(* Y-Axis *)\n");
    printf("yaxis size 3 min -12 max 12\n");//,max);
