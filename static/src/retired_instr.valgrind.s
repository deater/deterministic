	...
	# General Overhead =              31 instructions
...
_start:
	...
	xor	%r11,%r11   # gi7
	xor	%r12,%r12   # gi8
	xor	%r13,%r13   # gi9		# set r13 to one
	inc	%r13        # gi10
	...
	mov	$100000, %rcx		# gi11 load counter
...
.endif		
	...
	mov	$100000, %rcx		# g12 load counter
...
.if VALGRIND==9999	
	...
	leave		#i3 Load 1 # equal to mov ebp,esp ! pop ebp
	leave           #i4 Load 2
.endif	
...
string_loop:	
	...
	# test 16-bit laod ordered rep/size/lodsw
...
#.endif	
	...
	#   2,519,040,000 - 350,000 = 2,518,690,000
...
.endif
	...
	fxsave	buffer1             # i282 Load 60, Store 29
.if VALGRIND==999	
	fxrstor	buffer1             # i283 Load 61
.endif
...
mmx_test:
	...
	pand	%mm0,%mm1                   # i46
	pand	temp_quad,%mm1              # i47  Load 17
	...
	pandn	%mm0,%mm1                   # i50
	pandn	temp_quad,%mm1              # i51  Load 20
	...
	pslld	%mm0,%mm1                   # i62
	pslld	temp_quad,%mm1              # i63  Load 29
	pslld	$3,%mm0                     # i64
	...
	psllq	%mm0,%mm1                   # i65
	psllq	temp_quad,%mm1              # i66  Load 30
	psllq	$3,%mm0                     # i67
	...
	psllw	%mm0,%mm1                   # i68
	psllw	temp_quad,%mm1              # i69  Load 31
	psllw	$3,%mm0                     # i70
	...
	psrad	%mm0,%mm1                   # i71
	psrad	temp_quad,%mm1              # i72  Load 32
	psrad	$3,%mm0	                    # i73
	...
	psraw	%mm0,%mm1                   # i74
	psraw	temp_quad,%mm1              # i75  Load 33
	psraw	$3,%mm0		            # i76
	...
	psrld	%mm0,%mm1                   # i77
	psrld	temp_quad,%mm1              # i78  Load 34
	psrld	$3,%mm0			    # i79
	...
	psrlq	%mm0,%mm1                   # i80
	psrlq	temp_quad,%mm1              # i81  Load 35
	psrlq	$3,%mm0			    # i82
	...
	psrlw	%mm0,%mm1                   # i83
	psrlw	temp_quad,%mm1              # i84  Load 36
	psrlw	$3,%mm0			    # i85
	...
	paddb	%mm0,%mm1                   # i86
	paddb	temp_quad,%mm1              # i87  Load 37

	# paddd

	paddd	%mm0,%mm1                   # i88
	paddd	temp_quad,%mm1	            # i89  Load 38
	
	# paddsb

	paddsb	%mm0,%mm1                   # i90
	paddsb	temp_quad,%mm1              # i91  Load 39

	# paddsw

	paddsw	%mm0,%mm1                   # i92
	paddsw	temp_quad,%mm1	            # i93  Load 40
	
	# paddusb
	
	paddusb	%mm0,%mm1                   # i94
	paddusb	temp_quad,%mm1	            # i95  Load 41

	# paddusw

	paddusw	%mm0,%mm1                   # i96
	paddusw	temp_quad,%mm1		    # i97  Load 42
	
	# paddw
	
	paddw	%mm0,%mm1                   # i98
	paddw	temp_quad,%mm1		    # i99  Load 43

	# pmaddwd
	#   packed multiply, add doublewords
	
	pmaddwd	%mm0,%mm1                   # i100
	pmaddwd	temp_quad,%mm1		    # i101 Load 44

	# pmulhw
	#  packed multiply high signed word

	pmulhw	%mm0,%mm1                   # i102
	pmulhw	temp_quad,%mm1		    # i103 Load 45

	# pmullw
	#  packed multiply low signed word

	pmullw	%mm0,%mm1                   # i104
	pmullw	temp_quad,%mm1		    # i105 Load 46
	
	# psubb
	#  packed subtract bytes

	psubb	%mm0,%mm1                   # i106
	psubb	temp_quad,%mm1		    # i107 Load 47

	# psubd
	#  packed subtract doublewords
	
	psubd	%mm0,%mm1                   # i108
	psubd	temp_quad,%mm1		    # i109 Load 48
	
	# psubsb
	#   packed sub signed with saturation bytes

	psubsb	%mm0,%mm1                   # i110 
	psubsb	temp_quad,%mm1		    # i111 Load 49	

	# psubsw
	#   packed sub signed with saturation words

	psubsw	%mm0,%mm1                   # i112
	psubsw	temp_quad,%mm1              # i113 Load 50
	
	# psubusb
	#   packed sub unsigned and saturate bytes

	psubusb	%mm0,%mm1                   # i114
	psubusb	temp_quad,%mm1              # i115 Load 51
	
	# psubusw
	#   packed sub unsigned and saturate words

	psubusw	%mm0,%mm1                   # i116
	psubusw	temp_quad,%mm1              # i117 Load 52
	
	# psubw
	#   packed sub words

	psubw	%mm0,%mm1                   # i118
	psubw	temp_quad,%mm1	            # i119 Load 53
...
sse1_test:
	...
	pavgb  	%mm0,%mm1                           # i178
	pavgb	temp_quad,%mm0                      # i179 Load 107
	...
	pavgw  	%mm0,%mm1                           # i184
	pavgw	temp_quad,%mm0                      # i185 Load 111
	...
	pmaxsw  %mm0,%mm1                           # i190
	pmaxsw	temp_quad,%mm0                      # i191 Load 115
	...
	pmaxub  %mm0,%mm1                           # i196
	pmaxub	temp_quad,%mm0                      # i197 Load 119
	...
	pminsw  %mm0,%mm1                           # i202
	pminsw	temp_quad,%mm0                      # i203 Load 123
	...
	pminub  %mm0,%mm1                           # i208
	pminub	temp_quad,%mm0                      # i209 Load 127
	...
	pmulhuw %mm0,%mm1                           # i214
	pmulhuw	temp_quad,%mm0                      # i215 Load 131
	...
	psadbw %mm0,%mm1                            # i220
	psadbw	temp_quad,%mm0                      # i221 Load 135
	...
	movntq	%mm0,temp_128	                  # i294 Store 15
	...
	prefetcht0 0xdeadb000                     # i297 Load 162
		
	# prefetcht1
	#
	prefetcht1 temp_128                       # i298 Load 163
	prefetcht1 0xdeadb100	                  # i299 Load 164
	
	# prefetcht2
	#
	prefetcht2 temp_128                       # i300 Load 165
	prefetcht2 0xdeadb400		          # i301 Load 166
	
	### Prefetch, non-temporal (only into L2?)
	
	# prefetchnta
	prefetchnta temp_128                      # i302 Load 167
	prefetchnta 0xdeadb800		          # i303 Load 168
...
sse2_test:
	...
	pmuludq %mm0,%mm1                          # i274
	pmuludq	temp_quad,%mm0                     # i275 Load 162
	...
	psubq	%mm0,%mm1                          # i286
	psubq	temp_128,%mm1		           # i287 Load 170	
	...
	maskmovdqu	%xmm5,%xmm0                # i392 Store 18?
...
.endif
	...
	# iret iretd lar lds les lfs lgs lss lgdt lidt
...
os_test:
	...
	movl     $0xdeadbeef,(%rax)   # i16     Branch 3 Store 3 (+1i mem access)
